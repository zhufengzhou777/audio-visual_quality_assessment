...
from FusedSTAttention import FusedSTBlock
from model.vit_feature_extractor import MLP


class AVQA(nn.Module):
    def __init__(self, dim=768, fusion_dim=32, length=24):
        super().__init__()
        self.attn_layer = FusedSTBlock(dim=dim, num_heads=8)
        self.mlp = MLP(in_features=dim)
        self.gru = nn.GRU(dim, dim)
        self.regressor = MLP(in_features=dim, hidden_features=dim // 2, out_features=1)
        self.anormal = nn.LayerNorm(197)
        self.vnormal = nn.LayerNorm(197)
        self.tanh = nn.Tanh()
        self.relu = nn.ReLU()
        self.aencoder = nn.Linear(197, fusion_dim)
        self.vencoder = nn.Linear(197, fusion_dim)
        ...
        self.score = nn.Sequential(nn.LayerNorm(1536),
                                   nn.Linear(1536, 512),
                                   nn.ReLU(),
                                   nn.Dropout(),
                                   nn.Linear(512, 128),
    ...
    def forward(self, video, audio):
        cls_token = video[:, :, 0, :].unsqueeze(2)  # [B,F,1,768]
        st_token = video[:, :, 1:, :]  # [B,F,196,168]
        st_feat = self.attn_layer(st_token)  # [B,F,196,768]
        global_feat = self.mlp(cls_token)  # [B,F,1,768]
        visual_feat = torch.cat((st_feat, global_feat), dim=2)  # [B,F,197,768]
        visual_feat = torch.squeeze(self.regressor(visual_feat))  # [B,F,197]
        audio_feat = self.gru(audio)  # [B,F,197]
        ...
        J = torch.cat((X_a, X_v), dim=1)  # [24,64]
        d = J.shape[-1]
