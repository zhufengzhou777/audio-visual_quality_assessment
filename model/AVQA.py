...
class AVQA(nn.Module):
    def __init__(self, dim=768, fusion_dim=32, length=24):
        ...
        self.regressor1 = MLP(in_features=dim, hidden_features=dim // 2, out_features=1)
        self.regressor2 = MLP(in_features=dim, hidden_features=dim // 2, out_features=1)
    ...
    def forward(self, video, audio):
        ...
        visual_feat = torch.squeeze(self.regressor1(visual_feat))  # [B,F,197]
        ...
        audio_feat = torch.squeeze(self.regressor2(audio_feat))  # [B,F,197]
...
if __name__ == "__main__":
    avqa = AVQA()
    fit1 = torch.randn([2, 24, 197, 768])
    fit2 = torch.randn([2, 24, 197, 768])
