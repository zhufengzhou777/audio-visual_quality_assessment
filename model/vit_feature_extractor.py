...
def vit_feature_extractor(input, model_size='base'):
    """
    extract feature of [B,197,num_classes]
    """
    assert model_size in ['base', 'large']
    dict = {'base': 'jx_vit_base_patch16_224_in21k-e5005f0a.pth',
            'large': 'jx_vit_large_patch16_224_in21k-606da67d.pth'}
    if model_size == 'base':
        vit = VisionTransformer(num_classes=21843, embed_dim=768, depth=12, num_heads=12, representation_size=768)
        vit.load_state_dict(torch.load('pretrained_models/' + dict[model_size]))
    if model_size == 'large':
        vit = VisionTransformer(num_classes=21843, embed_dim=1024, depth=24, num_heads=16, representation_size=1024)
        vit.load_state_dict(torch.load('pretrained_models/' + dict[model_size]))
